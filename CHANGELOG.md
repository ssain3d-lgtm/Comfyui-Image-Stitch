...
- **Nothing is drawn over a card's picture any more.** The `⧉` duplicate, `×` remove, `‹ ›` steps and the `≡` handle are gone, and with them the blue capture-time badge that pushed the buttons off a narrow card; a card shows its number and its crop / rotation marks. **Click to edit, drag the card itself to reorder, right-click for the rest** — `Edit`, `Duplicate`, `Copy original`, `Replace` and `Remove`, each naming the card it acts on. A video card keeps its `×`, since a click on it opens the frame picker.
