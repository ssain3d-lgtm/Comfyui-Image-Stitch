...
- **Duplicate an image** (`⧉` on a card, or `Duplicate image #N` in the context menu): the same file once more, right after the one it came from, with its own crop and transform. Nothing is uploaded again, and the step is undoable.
...
- A card with a single image no longer draws the `≡` handle and the `‹ ›` steps: there is nothing to reorder, and a handle that ignores clicks reads as a broken button.
