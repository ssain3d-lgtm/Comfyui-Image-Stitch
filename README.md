...
- 이름은 클릭해서 바꿀 수 있고, 항목은 최대 200개까지 보관하며 넘치면 가장 오래 안 쓴 것부터 사라집니다. **`Load`·`Add`로 불러오는 것도 "사용"으로 쳐서** 맨 앞으로 올라오므로, 자주 꺼내 쓰는 구성이 밀려나지 않습니다. 항목은 `input/multi_stitch/gallery/`에 파일로 남아 ComfyUI를 재시작해도 그대로입니다.
